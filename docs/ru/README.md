...
### Вариант 0: API-ключ Kiro (Headless) — Рекомендуется

Самая простая настройка. Если у вас есть подписка Kiro **Pro / Pro+ / Power**,
сгенерируйте API-ключ (`ksk_...`) в консоли Kiro и укажите его шлюзу. Без входа через
браузер, без обновления токенов — ключ используется напрямую.

```env
# API-ключ Kiro (тот же ключ, что kiro-cli использует через KIRO_API_KEY в headless-режиме)
KIRO_API_KEY="ksk_..."
...
```bash
# Или встроенно, без редактирования .env:
KIRO_API_KEY="ksk_..." PROXY_API_KEY="my-super-secret-password-123" python main.py
```

> **Примечание:** `KIRO_API_KEY`, если задан, имеет приоритет над другими вариантами ниже.
> Регион по умолчанию — `us-east-1` (переопределяется через `KIRO_API_REGION`). Актуальный
> список моделей (включая новейшие) загружается автоматически.
